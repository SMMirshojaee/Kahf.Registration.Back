...
namespace Registration.API.Controllers
{
	...
	public class OrderController(
		...
		ILoggerFactory lg) :
		GenericController<OrderBusiness, Order>(business, mapper, appSetting, contextAccessor,lg)
	{
		...
		public async Task<IActionResult> RequestLoan(int regStepId, int loan, int cash)
		{
				...
				if (payment.LoanStatusId.HasValue)
				{
					...
					if (!report.Successful)
						return InternalServerError(
							"وام شما ثبت گردید اما وضعیت شما در سامانه ثبت نشد. این مشکل را به مدیر سامانه اطلاع دهید");
				}
				else
				{
			...
			ZarrinpalResponse requestReport = await paymentService.SendRequest(id: ApplicantId,
				firstName: applicant.FirstName, lastName: applicant.LastName, amount: cashOrder.Amount,
				orderId: cashOrder.Id, mobile: Mobile, devMode: env.IsDevelopment());
			if (!requestReport.Successful)
			{
				await Business.UpdateRequest(cashOrder.Id, requestReport.Authority, requestReport.Content,
					requestReport.Code);
				return InternalServerError("خطا در ارتباط با درگاه بانکی");
			}

			cashReport = await Business.UpdateRequest(cashOrder.Id, requestReport.Authority, requestReport.Content,
				requestReport.Code);
		...
		public async Task<IActionResult> SendRequestByRegStepId(int regStepId)
		{
			...
			ZarrinpalResponse requestReport = await paymentService.SendRequest(id: ApplicantId,
				firstName: applicant.FirstName, lastName: applicant.LastName, amount: newOrder.Amount,
				orderId: newOrder.Id, mobile: Mobile, devMode: env.IsDevelopment());
			if (!requestReport.Successful)
			{
				await Business.UpdateRequest(newOrder.Id, requestReport.Authority, requestReport.Content,
					requestReport.Code);
				return InternalServerError("خطا در ارتباط با درگاه بانکی");
			}

			report = await Business.UpdateRequest(newOrder.Id, requestReport.Authority, requestReport.Content,
				requestReport.Code);
			return Ok(requestReport.RedirectUrl);
		}
		...
		public async Task<IActionResult> Callback([FromQuery] string authority, [FromQuery] string status)
		{
			...
			if (!order.RegStepId.HasValue)
			{
				...
				else
				{
					...
					await Business.SaveChanges();
					return Redirect(
						$"{AppSetting.FrontPaymentPage}?authority={authority}&messageCode=0&refId={verifyResponse.RefId}");
				}
			}
