...
public partial class ApplicantController(
	...
	ApplicantExtraCostBusiness applicantExtraCostBusiness,
	SmsHelper smsSender,
	RegStepStatusBusiness regStepStatusBusiness,
	ApplicantFormValueBusiness applicantFormValueBusiness,
	...
	ILoggerFactory lg) :
	GenericController<ApplicantBusiness, Applicant>(b, m, ap, ac, lg)
{
	[HttpPost("{regId}")]
	[AllowAnonymous]
	...
	public async Task<IActionResult> SingIn(int regId, SigninDto signinForm)
		=> Status(await Business.SingIn(regId, signinForm.NationalCode, signinForm.Mobile, signinForm.TrackingCode,
			AppSetting));
	...
	public async Task<IActionResult> AddMember(int regStepId, SignupDto addMemberForm)
		=> Status(await Business.AddMember(ApplicantId, regStepId, addMemberForm.FirstName, addMemberForm.LastName,
			addMemberForm.NationalCode, addMemberForm.Mobile));
