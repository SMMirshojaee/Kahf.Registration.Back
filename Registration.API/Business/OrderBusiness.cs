...
namespace Registration.API.Business
{
	public class OrderBusiness(RegContext context, IMapper mapper) : GenericBusiness<Order>(context, mapper)
	{
		...
		internal async Task<ActionReport<OrderDto>> AddNewOrder(int userId, OrderDto newCash)
		{
			Order? newOrder = Mapper.Map<Order>(newCash);
			newOrder.UserId = userId;
			newOrder.NationalNumber = newCash.NationalNumber;
			newOrder.RegStepId = null;
			newOrder.VerifyDate = newOrder.RequestDate = newCash.VerifyDate;
			newOrder.RequestStatus = newOrder.VerifyStatus = 100;
			ActionReport report = await Add(newOrder);
			if (report.Successful)
			{
